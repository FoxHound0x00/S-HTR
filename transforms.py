...
class GaussianBlur(object):
    ...
    def __init__(self, kernel_size, sigma=(0.1, 2.0)):
        ...
        self.sigma = sigma
        self.gaussian_blur = v2.GaussianBlur(self.kernel_size, self.sigma)
        
    def __call__(self, sample):
        image, label = sample['image'], sample['label']
        return {'image': self.gaussian_blur(image), 'label': label}
...
class RandomErasing(object):
    ...
    def __call__(self, sample):
        image, label = sample['image'], sample['label']
        return {'image': self.random_erasing(image), 'label': label}